...
with gr.Blocks(title="智能相册", theme=gr.themes.Soft()) as app:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            upload = gr.Image(label="上传图片", type="pil")  # 使用 gr.Image 上传图像，类型为 PIL 图像
        ...
        def update_gallery():
            """更新图像库"""
            # 获取保存目录中的所有图像文件路径
            image_files = [os.path.join(SAVE_DIR, f) for f in os.listdir(SAVE_DIR) if f.endswith((".png", ".jpg", ".jpeg"))]
        ...
        def handle_upload(image):
            ...
            return f"图像已保存到: {saved_path}", updated_gallery  # 返回文本和更新后的图像列表
        upload.upload(
            fn=handle_upload,
            inputs=upload,
            outputs=[
